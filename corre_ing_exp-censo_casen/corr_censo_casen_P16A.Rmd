...
abstract: |
  Calculamos correlaciones entre el ingreso promedio comunal **multiplicado por la población también comunal** que llamaremos **ingresos expandidos** extraídos de la Casen 2017 y las frecuencias de respuesta por categoría a la pregunta **P16A** del Censo de viviendas 2017, también extraídas a nivel comunal. Lo haremos a nivel nacional.
...
## Pregunta **P16A**: Pueblo indígena u originario listado
...
##  Generación de tabla de contingencia para la variable P16A
...
write_xlsx(union_final_urb, "P16A_urbano.xlsx")
```
...
saveRDS(tabla,"tablas_de_corr/C_P16A_URB.rds")  
tabla %>%
...
```{r, attr.source='.numberLines', message=FALSE, warning=FALSE}
union_final_urb <- df_2017_2[,-c(1,(ncol(df_2017_2)-1))]
write_xlsx(union_final_urb, "P16A_rural.xlsx")
```
...
tabla <- as.data.frame(tabla)
colnames(tabla) <- "Correlación"
saveRDS(tabla,"tablas_de_corr/C_P16A_RU.rds")  
tabla %>%
  rownames_to_column("Pueblo indígena u originario listado") %>%  
  mutate(Correlación = cell_spec(Correlación, background=ifelse(Correlación == max(Correlación), "#fc0303", "#5cb81f"))) %>% 
  kbl(booktabs = T, linesep = "", escape=FALSE) %>% 
