...
```{r, warning = FALSE}
df_2017_2 <- df_2017_2[,-c(2,4,6,8,10,12,14,15,16,17,18)]

```


```{r, warning = FALSE}
write_xlsx(df_2017_2,"P03A_urbano.xlsx")
...
```{r}
df_2017_3 <- df_2017_2[,-c(1)]
```


```{r}
data_sum <- summary(df_2017_3 )
kbl(head(data_sum)) %>%
  kable_styling(bootstrap_options = c("striped", "hover")) %>%
  kable_paper() %>%
...
chart.Correlation(df_2017_3, histogram=TRUE, method = c( "kendall"), pch=20)
```
...
chart.Correlation(df_2017_3, histogram=TRUE, method = c( "pearson"), pch=20)
```
...
chart.Correlation(df_2017_3, histogram=TRUE, method = c( "spearman"), pch=20)
```
