...
##  Generación de ingresos expandidos a nivel Urbano (de ahora en adelante sólo llamaremos al rds ya construído):
...
df_2017_2 = merge( x = comuna_corr, y = df_2017, by = "código", all.x = TRUE)
...
## Union de frecuencias con ingresos expandidos:
...
generacion de tabla a correlacionar:

```{r}
#union_final_urb_corr  <- union_final_urb[,-c(1)]

write_xlsx(union_final_urb, "P01.xlsx")
```
...
df_2017_fig <- union_final_urb_corr[,-c((ncol(union_final_urb_corr)-2),(ncol(union_final_urb_corr)-1),(ncol(union_final_urb_corr)))]


fig <- plot_ly(df_2017_fig, x = `código`, y = df_2017_fig[,2]
