...
abstract: |
  Calculamos correlaciones entre el ingreso promedio comunal **multiplicado por la población también comunal** que llamaremos **ingresos expandidos** extraídos de la Casen 2017 y las frecuencias de respuesta por categoría a la pregunta **P16** del Censo de viviendas 2017, también extraídas a nivel comunal. Lo haremos a nivel nacional.
...
## Pregunta **P16**: Se considera perteneciente a un pueblo indígena u originario
...
##  Generación de tabla de contingencia para la variable P16
...
write_xlsx(union_final_urb, "P16_urbano.xlsx")
```
...
saveRDS(tabla,"tablas_de_corr/C_P16_URB.rds")
tabla %>%
...
```{r, attr.source='.numberLines', message=FALSE, warning=FALSE}
union_final_urb <- df_2017_2[,-c(1,(ncol(df_2017_2)-1))]
write_xlsx(union_final_urb, "P16_rural.xlsx")
```
...
tabla <- tabla %>% drop_na()
colnames(tabla) <- "Correlación" 
saveRDS(tabla,"tablas_de_corr/C_P16_RU.rds") 
tabla %>%
  rownames_to_column("se considera perteneciente a un pueblo  indígena u originario") %>%  
  mutate(Correlación = cell_spec(Correlación, background=ifelse(Correlación == max(Correlación), "#fc0303", "#5cb81f"))) %>% 
  kbl(booktabs = T, linesep = "", escape=FALSE) %>% 
