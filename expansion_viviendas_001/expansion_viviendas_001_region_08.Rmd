---
title: Expansión de la CASEN sobre el CENSO (Región 08)
author:
- name: VE-CC-AJ
...
## 9 Modelo raíz-log (**log-log**)

El modelo que nos entrega el mayor coeficiente de determinación de todos (0,8199) es el raíz-log, sin embargo, el coeficiente de intersección no resulta estadísticamente significativo (su Pr es 0,278). Es por ésta razón que preferimos el modelo log-log cuyos coeficientes sí son significativos poseyendo un coeficiente de determinación de 0,8127.
...
```{r}
h_y_m_comuna_corr_01$est_ing <- exp(15.84389+0.96276 * log(h_y_m_comuna_corr_01$Freq.x))
...
```{r}
saveRDS(h_y_m_comuna_corr_01, "casen_censo_region_08.rds")
```
