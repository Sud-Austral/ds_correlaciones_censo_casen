...
```{r}
ingresos_expandidos_rurales <- readRDS("C:\\Users\\chris\\OneDrive\\Documentos\\GitHub\\ds_correlaciones_censo_casen\\reg_ce_ca_2017\\urbano\\ingresos_expandidos_casen_2017_totales_r.rds")
...
### se pierde la primera ronda por ausencia de filas de Casen
...
### aqui excluimos los outliers de frec de resp.
...
```{r, warning = FALSE, message = FALSE}
ggplot(casen_2017_sin_o, aes(x = sqrt(frecuencia_de_resp), y = sqrt(multipob))) + 
  geom_point() +
  stat_smooth(method = "lm", col = "red")
```
...
```{r}
scatter.smooth(x=tabla_5$frecuencia_de_resp, y=tabla_5$multipob, main="ing_medio_zona ~ Freq",
     xlab = "Freq",
     ylab = "multi_pob",
           col = 2, is.na = T)
```
...
```{r}
# Q <- quantile(casen_2017_sin_o$frecuencia_de_resp, probs=c(.25, .75), na.rm = T)
# iqr <- IQR(casen_2017_sin_o$frecuencia_de_resp, na.rm = T)
# casen_2017_sin_o <- subset(tabla_sin_out,casen_2017_sin_o$frecuencia_de_resp > (Q[1] - 1.5*iqr) & casen_2017_sin_o$frecuencia_de_resp < (Q[2] + 1.5*iqr))
# casen_2017_sin_o <- data.frame(lapply(casen_2017_sin_o, as.character), stringsAsFactors=FALSE)
# casen_2017_sin_o$frecuencia_de_resp <- as.numeric(casen_2017_sin_o$frecuencia_de_resp)
# casen_2017_sin_o$multipob <- as.numeric(casen_2017_sin_o$multipob)
# casen_2017_sin_o
...
```{r}
ggplot(data = casen_2017_sin_o) + geom_boxplot(aes(x=ing_medio_zona, y=ing_medio_zona))
```
