...
Aplicar un análisis de regresión entre la variable MUTIPOB_mz y la frecuencia de la variable X (o su proporción respecto al total de viviendas por zona en caso de que no esté) a nivel de manzana, y obtener el modelo que arroje el coeficiente de determinación más alto.


```{r}
union_final <- readRDS("urbano_rural_final.rds")
```
...
```{r}
union_final <- filter(union_final, union_final$identificador == "region_01") 
union_final <- filter(union_final, union_final$urb_rur== "1") 
```
...
```{r}
scatter.smooth(x=union_final$Freq.x, y=union_final$multipob, main="multi_pob ~ Freq.x",
     xlab = "Freq.x",
     ylab = "multi_pob",
           col = 2) 
```
