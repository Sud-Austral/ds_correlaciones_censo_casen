...
## 5 Modelo lineal sobre **p_variable**
...
### 5.1 Aplicación la regresión a los valores de la variable a nivel de zona
...
```{r}
modelo_lineal <- h_y_m_comuna_corr_01
```



```{r}
modelo_lineal$est_ing <- 6.198e+08 + 1.865e+10* modelo_lineal$p_variable
```

```{r}
r3_100 <- modelo_lineal[c(1:100),]
kbl(r3_100) %>%
  kable_styling(bootstrap_options = c("striped", "hover")) %>%
  kable_paper() %>%
  scroll_box(width = "100%", height = "300px")
...
### 5.2 División del valor estimado entre la población total de la zona para obtener el ingreso medio por zona
<br>
...
```{r}
modelo_lineal$ing_medio_zona <- modelo_lineal$est_ing  /( modelo_lineal$personas  * modelo_lineal$p_poblacional)
```
...
## 6 Modelos sobre **Freq.x**

### 6.1 Modelo cuadrático
...
Debemos reemplazar los NAs de los registros del campo Freq.x y lo hacemos con los valores promedio, generando una nueva columna llamada Freq.x_mean:.
...
```{r}
h_y_m_comuna_corr_01$multi_pob_without_na <- ifelse(is.na(h_y_m_comuna_corr_01$multi_pob),
...
```{r}
any(is.na(h_y_m_comuna_corr_01$multi_pob_without_na))
```

Hay que investigar por qué no corre el modelo para el exponencial.

```{r}
# linearMod <- lm(multi_pob_without_na~exp(Freq.x) , data=h_y_m_comuna_corr_01)
# summary(linearMod)
```

### 6.5 Modelo con raíz cuadrada (**mrc**)
...
### 6.6 Diagrama de dispersión sobre **mrc**
...
### 6.9 Ecuación del modelo

<br>

$$ \hat Y = -136885689 + 83356602 \cdot \sqrt{X}  $$

<br>

### 6.10 Aplicación la regresión a los valores de la variable a nivel de zona

Esta nueva variable se llamará: est_ing

```{r}
h_y_m_comuna_corr_01$est_ing <- -136885689 + 83356602*  sqrt(h_y_m_comuna_corr_01$Freq.x)
...
### 6.11 División del valor estimado entre la población total de la zona para obtener el ingreso medio por zona
