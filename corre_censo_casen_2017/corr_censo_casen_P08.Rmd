...
abstract: |
  Calculamos correlaciones entre el ingreso promedio comunal **multiplicado por la población también comunal** que llamaremos **ingresos expandidos** extraídos de la Casen 2017 y las frecuencias de respuesta por categoría a la pregunta **P08** del Censo de viviendas 2017, también extraídas a nivel comunal. Lo haremos a nivel nacional.
...
##  Generación de tabla de contingencia para la variable P08
...
names(comuna_corr)[2] <- "Hombre"
names(comuna_corr)[4] <- "Mujer"
...
write_xlsx(union_final_urb, "P08_urbano.xlsx")
```
...
saveRDS(tabla,"tablas_de_corr/C_P08_URB.rds") 
tabla %>%
...
```{r, attr.source='.numberLines', message=FALSE, warning=FALSE}
union_final_urb <- df_2017_2[,-c(1,(ncol(df_2017_2)-1))]
write_xlsx(union_final_urb, "P08_rural.xlsx")
```
...
tabla <- tabla %>% drop_na()
colnames(tabla) <- "Correlación"  
saveRDS(tabla,"tablas_de_corr/C_P08_RU.rds") 
tabla %>%
  rownames_to_column("Sexo") %>%  
  mutate(Correlación = cell_spec(Correlación, background=ifelse(Correlación == max(Correlación), "#fc0303", "#5cb81f"))) %>% 
  kbl(booktabs = T, linesep = "", escape=FALSE) %>% 
