...
```{r}
length(unique(claves_con_1$clave))
```
...
determinar las comunas

```{r}
lista_de_comunas <- unique(tabla_con_clave_f$COMUNA)
```
...
e <- table(tabla_con_clave$clave, tabla_con_clave$P17) 
...
```{r}
filtro_e <- filter(e, e$Var2 == 1)
filtro_e
```


```{r}
filter(filtro_e, filtro_e$Freq == 0)
```

Exactamente hay 48 zonas que no poseen data 1
...
* Cuantas claves existen que no posean la respuesta 1?





** se aproximan los ingresos estimadosa los reales?

```{r}
urc <- read_xlsx("urbano_rural_corregido.xlsx")
```
...
```{r}
losvilos <- filter(urc, urc$código.x == 4203)
losvilos
```

```{r}
mean(losvilos$ing_medio_zona)
```
...
```{r}
urc2 <- read_xlsx("urbano_rural_corregido.xlsx")
#urc2 <- filter(urc2, urc2$`código.x`== "4102") 
urc2 <- filter(urc2, urc2$urb_rur== "1") 
urc2
```
...
```{r}
saveRDS(comunas_censo_casen, "URBANO/region_01_P17_u.rds")
```
...
## Anexo: Integriedad de los datos estimados.

```{r}
region_01 <- readRDS("URBANO/region_01_P17_u.rds")
region_01
```
...
Creeemos que existen respuestas a las pregunta P17 ¿Trabajó por un pago o especie? en la base de datos original.
