...
Es éste el modelo que nos entrega el mayor coeficiente de determinación de todos (0.9808).
...
```{r}
scatter.smooth(x=sqrt(h_y_m_comuna_corr$Freq.x), y=sqrt(h_y_m_comuna_corr$multi_pob), lpars = list(col = "red", lwd = 2, lty = 1), main="multi_pob ~ Freq.x")
```
...
```{r}
linearMod <- lm( sqrt(multi_pob)~sqrt(Freq.x) , data=h_y_m_comuna_corr)
summary(linearMod) 
```

```{r, warning = FALSE, message = FALSE}
ggplot(h_y_m_comuna_corr, aes(x = sqrt(Freq.x) , y = sqrt(multi_pob))) + geom_point() + stat_smooth(method = "lm", col = "red")
```
...
<br>
 ...
 $$\hat Y = {-309.2197}^2 + 2  -309.2197  942.8527 \sqrt{X}+  942.8527^2 X  $$
...
```{r}
saveRDS(h_y_m_comuna_corr, "P03C/region_15_P03C_u.rds")
```
