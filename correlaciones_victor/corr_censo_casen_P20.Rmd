...
## 1 Pregunta **P20**: Total hijos/as actualmente vivos

0-23

### 1.1 Cálculo de frecuencias

Leemos las respuestas a la pregunta P20 del censo de **personas** 2017 y obtenemos la tabla de frecuencias por categoría de respuesta:

```{r, warning = FALSE}
tabla_con_clave <- readRDS("../censo_personas_con_clave_17")
tabla_con_clave <- filter(tabla_con_clave, tabla_con_clave$AREA == 1)
b <- tabla_con_clave$COMUNA
c <- tabla_con_clave$P20
...
```
Solamente podemos establecer correlaciones hasta los 16 hijos, mas allá no podemos por la presencia masiva de **NAs**

```{r}

kbl(df_2017_2[,c(54:73)]) %>%
  kable_styling(bootstrap_options = c("striped", "hover")) %>%
  kable_paper() %>%
  scroll_box(width = "100%", height = "300px")
```
...
## 2 Pregunta **P20**: Total hijos/as actualmente vivos
...
chart.Correlation(my_data, histogram=TRUE, method = c( "pearson"), pch=20)
```
Solamente podemos establecer correlaciones hasta los 17 hijos, mas allá no podemos por la presencia masiva de **NAs**

```{r}

kbl(df_2017_2[,c(57:73)]) %>%
  kable_styling(bootstrap_options = c("striped", "hover")) %>%
  kable_paper() %>%
  scroll_box(width = "100%", height = "300px")
```
