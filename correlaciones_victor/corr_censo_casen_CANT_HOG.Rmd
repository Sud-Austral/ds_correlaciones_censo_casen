...
## 1 Pregunta **CANT_HOG**: Cantidad de hogares
...
```{r, warning = FALSE}
III <- seq(3,48,3)
my_data <- df_2017_2[, c(III,118)]
chart.Correlation(my_data, histogram=TRUE, method = c( "pearson"), pch=20)
```
Solamente podemos establecer correlaciones hasta los 15 hogares dentro de una vivienda, mas allá no podemos por la presencia masiva de **NAs**

```{r}

kbl(df_2017_2[,c(51:117)]) %>%
  kable_styling(bootstrap_options = c("striped", "hover")) %>%
  kable_paper() %>%
  scroll_box(width = "100%", height = "300px")
```
...
## 2 Pregunta **CANT_HOG**: Cantidad de hogares
...
chart.Correlation(my_data, histogram=TRUE, method = c( "pearson"), pch=20)
```
Si bien la correlacion mas alta da *0.49* en el caso de 6 hogares con ingresos expandidos, es ésta, espúrea por la cantidad mínima de datos a correlacionar.
